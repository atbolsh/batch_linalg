...
int batch_solver_single(float* bA, float* bb, float* bx, int b, int n)
{
    ...
    float *d_b = NULL; // batchSize * m
    float *d_x = NULL; // batchSize * m
...
// step 3: copy Aj and bj to device
    cudaStat1 = cudaMalloc ((void**)&d_csrValA   , sizeof(float) * nnzA * b);
    cudaStat2 = cudaMalloc ((void**)&d_csrColIndA, sizeof(int) * nnzA);
    cudaStat3 = cudaMalloc ((void**)&d_csrRowPtrA, sizeof(int) * (n+1));
    cudaStat4 = cudaMalloc ((void**)&d_b         , sizeof(float) * n * b);
    cudaStat5 = cudaMalloc ((void**)&d_x         , sizeof(float) * n * b);
    ...
    cudaStat1 = cudaMemcpy(d_csrValA   , bA,         sizeof(float) * nnzA * b, cudaMemcpyHostToDevice);
    ...
    cudaStat4 = cudaMemcpy(d_b,          bb, sizeof(float) * n * b, cudaMemcpyHostToDevice);
    assert(cudaStat1 == cudaSuccess);
    assert(cudaStat2 == cudaSuccess);
    assert(cudaStat3 == cudaSuccess);
    assert(cudaStat4 == cudaSuccess);
...
// step 7: Copy and return
// xBatch = [x0, x1, x2, ...]
    cudaStat1 = cudaMemcpy(bx, d_x, sizeof(float)*n*b, cudaMemcpyDeviceToHost);
    assert(cudaStat1 == cudaSuccess);
...
//Useful in debugging.
int print_em_single(float* d_bA, float* d_bb, float* d_bx, int b, int n)
{
    float* c_bA = (float*)malloc(sizeof(float)*b*n*n);
    float* c_bb = (float*)malloc(sizeof(float)*b*n);
    float* c_bx = (float*)malloc(sizeof(float)*b*n);

    cudaMemcpy(c_bx, d_bx, sizeof(float)*n*b,   cudaMemcpyDeviceToHost);
    cudaMemcpy(c_bb, d_bb, sizeof(float)*n*b,   cudaMemcpyDeviceToHost);
    cudaMemcpy(c_bA, d_bA, sizeof(float)*n*n*b, cudaMemcpyDeviceToHost);
